    ...
    <TitleNav bgc-color="#fff">
      ...
      >
        ...
        <cube-form-item :field="schema.groups.fields[0]">
          ...
          <div class="time-show" @click="selectTime">
            {{ model.time || "请选择" }}
        ...
        <cube-form-item :field="schema.groups.fields[1]">
          <!--            @click="showTimePicker"-->
          <div class="time-show" @click="selectType">
            {{ model.type || "请选择" }}
            <i class="cubeic-arrow" style="float: right"></i>
          </div>
        </cube-form-item>
...
export default {
  ...
  created() {
    ...
    this.timeFrom = [
      ...
      {
        text:
          WEEK_MAP[this.$dayjs().add(1, "day").day()] +
          " " +
          this.$dayjs().add(1, "day").format("MM月DD日"),
        value: this.$dayjs().add(1, "day").format("MM月DD日"),
      },
      {
        text:
          WEEK_MAP[this.$dayjs().add(2, "day").day()] +
          " " +
          this.$dayjs().add(2, "day").format("MM月DD日"),
  ...
  methods: {
    ...
    ...mapActions("printing", ["queryByTypeAndDateForPrintingRoom"]),
    ...mapActions("warehouse", ["queryByTypeAndDateForStoreHouse"]),
    async submitHandler(e, val) {
      ...
      if (type === "lifashi") {
        ...
        await this.queryByTypeAndDateForPrintingRoom({
          type: val.type,
          date: this.$dayjs().format("YYYY-") + this.model.time,
        });
      } else if (type === "kufang") {
        await this.queryByTypeAndDateForStoreHouse({
          type: val.type,
          date: this.$dayjs().format("YYYY-") + this.model.time,
        });
      }
      await this.$router.push({
        name: "ReservePage",
        params: { item: this.$route.params.id, type: type },
      });
    },
    selectType() {
      if (!this.TypePicker) {
    ...
    selectTime() {
      if (!this.TimePicker) {
        this.TimePicker = this.$createPicker({
          ...
          onSelect: (selectedVal, selectedIndex, selectedText) => {
            this.model.time =
              selectedVal[0].substr(0, 2) + "-" + selectedVal[0].substr(3, 2);
          },
        });
      }
