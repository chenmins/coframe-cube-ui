    ...
    <TitleNav bgc-color="#fff">
      ...
      >
        ...
        <cube-form-item :field="schema.groups.fields[1]">
          <!--            @click="showTimePicker"-->
          <div class="time-show" @click="selectStartTime">
            {{ model.startTime || "请选择" }}
            <i class="cubeic-arrow" style="float: right; margin-right: 16px"></i>
          </div>
        </cube-form-item>
        <cube-form-item :field="schema.groups.fields[2]">
          <!--            @click="showTimePicker"-->
          <div class="time-show" @click="selectEndTime">
            {{ model.endTime || "请选择" }}
            <i class="cubeic-arrow" style="float: right; margin-right: 16px"></i>
          </div>
        </cube-form-item>
...
export default {
  ...
  methods: {
    ...
    ...mapActions("printing", ["addPrintingRoom"]),
    ...mapActions("warehouse", ["addStoreHouse"]),
    validateHandler() {
    ...
    async selectedType(data) {
      switch (this.releaseType) {
        ...
        case "restaurant":
          ...
          break;
        case "pritingRoom":
          return await this.addPrintingRoom(data);
          break;
        case "kufang":
          return await this.addStoreHouse(data);
          break;
        default:
          break;
      }
